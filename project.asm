...
.data
MAX_ACCOUNTS EQU 4
MAX_DAILY_WITHDRAWAL EQU 1000
MAX_SINGLE_WITHDRAWAL EQU 500
...
Account STRUCT
	...
	cardType BYTE ? ;BRONZE=0/SILVER=1/GOLD=2
...
accountsFile BYTE "accounts.dat",0
atmCashFile BYTE "atmcash.dat",0
transactionsFile BYTE "transactions.txt",0
fileHandle DWORD ?
bytesWritten DWORD ?
fileErrorMsg BYTE "Error accessing files!",13,10,0
fileSavedMsg BYTE "Data saved successfully.",13,10,0
...
invalidChoice BYTE 13,10,
        ...
        "           Please select from 1-5!",13,10,0
...
CheckAccountsLoop:
    ...
    je FileNotFound
    add esi,SIZEOF Account
    loop CheckAccountsLoop
    ret

FileNotFound:
    call InitializeDefaultAccounts
    ret
LoadAccountsFromFile ENDP

SaveAccountsToFile PROC
    INVOKE CreateFile,
        OFFSET accountsFile,
        GENERIC_WRITE,
        0,
...
SaveError:
    mov edx,OFFSET fileErrorMsg
      call WriteString
      ret
SaveAccountsToFile ENDP

;--------Load atm cash from file-----------
LoadATMCashFromFile PROC
    INVOKE CreateFile,
        OFFSET atmCashFile,
        GENERIC_READ,
        0,
...
SetDefaultCash:
    ...
    mov atm10, 50
    ret
LoadATMCashFromFile ENDP

SaveATMCashToFile PROC
    INVOKE CreateFile,
        OFFSET atmCashFile,
        GENERIC_WRITE,
        0,
...
SaveError:
      ...
      call WriteString
      ret
SaveATMCashToFile ENDP

SaveTransactions PROC
    push eax
    push ebx
    push esi
    INVOKE CreateFile,
    OFFSET transactionsFile,
        GENERIC_WRITE,
        0,
        0,
        OPEN_ALWAYS,
     ...
     INVOKE SetFilePointer,
        ...
        0,
        FILE_END
        ;format: ACC:1001 , TYPE:W AMOUNT:500 ,BALANCE:4500
        INVOKE CloseHandle, fileHandle
TransError:
    pop esi
    pop ebx
    pop eax
    ret
SaveTransactions ENDP
InitializeDefaultAccounts PROC
    mov esi, OFFSET accounts
    ;ACCOUNT 1
    ...
    add esi,SIZEOF Account
    ;ACCOUNT 2
    mov DWORD PTR [esi+0], 1002    ; accountNumber
    mov DWORD PTR [esi+4], 5678     ; pin
    mov DWORD PTR [esi+8], 10000      ; balance
    ...
    add esi,SIZEOF Account
    ;ACCOUNT 3
    mov DWORD PTR [esi+0], 1003      ; accountNumber
    mov DWORD PTR [esi+4], 9090      ; pin
    mov DWORD PTR [esi+8], 15000      ; balance
    ...
    mov BYTE PTR [esi+52],0
    ret
InitializeDefaultAccounts ENDP
...
InvalidTechPin:
    mov eax,12
    call SetTextColor
    mov edx,OFFSET wrongTechPinMsg
    call WriteString
    mov eax,2000
    call Delay
    jmp StartATM
